tgarch_grab_pars <- function(theta, p, q, incl_mean, extra_par, skew_par) {
...
sigt_tgarch_R <- function(theta, x, p, q, p_ar, q_ma,
              incl_mean, extra_par, skew_par,
              trunc, presample, ...) {

  list2env(list(...), envir = environment())

  all_pars <- tgarch_grab_pars(
    theta = theta,
    p = p,
    q = q,
    incl_mean = incl_mean,
...
goal_fun_creator_tgarch <- function(
    theta,
    x,
    pdf_fun2,
    p,
...
#'
#'Fit a TGARCH model under the six
...
#'\deqn{\sigma_t=\omega+\sum_{i=1}^{p}\phi_i\left(\left|\varepsilon_{t-i}\right|-\gamma_{i}\varepsilon_{t-i}\right) + \sum_{j=1}^{j} \beta_{j}\sigma_{t-j}.}
...
#'
#'@return
#'An object of S4-class \code{"fEGarch_fit_tgarch"}
...
#'}
#'
tgarch <- function(rt, orders = c(1, 1),
...
#'@export
#'@rdname show-methods
#'@aliases show,fEgarch_fit_tgarch-method
setMethod(
  "show",
  "fEGarch_fit_tgarch",
  function(object) {
