gjrgarch_grab_pars <- function(theta, p, q, incl_mean, extra_par, skew_par) {
  ...
  shape <- if (extra_par) theta[[step3 + 1]] else 0
  skew <- if (skew_par) theta[[step3 + 1 + extra_par]] else 0
...
sigt_gjrgarch_R <- function(theta, x, p, q, p_ar, q_ma,
              incl_mean, extra_par, skew_par,
              trunc, presample, ...) {

  list2env(list(...), envir = environment())

  all_pars <- gjrgarch_grab_pars(
    theta = theta,
    p = p,
    q = q,
    incl_mean = incl_mean,
  ...
  sigt <- c(sigt_aparch_Cpp(    # keep APARCH function here and use fixed delta
    x = x,
    mu = mu,
    phi = phi,
    beta = beta,
...
goal_fun_creator_gjrgarch <- function(
    theta,
    x,
    pdf_fun2,
    p,
...
#'
#'Fit a GJR-GARCH model under the six
...
#'\deqn{\sigma_t^{2}=\omega+\sum_{i=1}^{p}\phi_i\left(\left|\varepsilon_{t-i}\right|-\gamma_{i}\varepsilon_{t-i}\right)^{2} + \sum_{j=1}^{j} \beta_{j}\sigma_{t-j}^{2}.}
...
#'
#'See also the reference section for sources on the GJR-GARCH (Glosten et al., 1993),
#'the more general
...
#'\eqn{\gamma_1,\dots,\gamma_p}, shape parameter,
...
#'
#'@return
#'An object of S4-class \code{"fEGarch_fit_gjrgarch"}
...
#'}
#'
gjrgarch <- function(rt, orders = c(1, 1),
...
#'@export
#'@rdname show-methods
#'@aliases show,fEgarch_fit_gjrgarch-method
setMethod(
  "show",
