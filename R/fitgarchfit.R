fitgarch_grab_pars <- function(theta, p, q, incl_mean, extra_par, skew_par) {
  ...
  gamma <- theta[[step2 + 1]]
  d <- theta[[step2 + 2]]
  shape <- if (extra_par) theta[[step2 + 3]] else 0
  skew <- if (skew_par) theta[[step2 + 3 + extra_par]] else 0
...
sigt_fitgarch_R <- function(theta, x, p, q, p_ar, q_ma,
              incl_mean, extra_par, skew_par,
              trunc, presample, ...) {

  list2env(list(...), envir = environment())

  all_pars <- fitgarch_grab_pars(
    theta = theta,
    p = p,
    q = q,
    incl_mean = incl_mean,
...
goal_fun_creator_fitgarch <- function(
    theta,
    x,
    pdf_fun2,
    p,
...
#'
#'Fit a fractionally integrated TGARCH model under the six
...
#'
#'@details
#'Consider a FITGARCH(\eqn{p, d, q}) (Ayensu et al., 2025) with constant asymmetry term \eqn{\gamma} regardless
...
#'\deqn{\sigma_t=\omega+\left[1-\beta^{-1}(B)\phi(B)(1-B)^{d}\right]\left(\left|\varepsilon_t\right|-\gamma\varepsilon_t\right).}
...
#'to a short-memory TGARCH, for \eqn{d=1} we have a full integration, and for
...
#'
#'The pre-sample values of \eqn{\left(\left|\varepsilon_t\right|-\gamma\varepsilon_t\right)}
#'are replaced by the in-sample arithmetic mean of
#'\deqn{\left(\left|r_t^{*} - \bar{r}_t^{*}\right|-
#'\gamma_0\left(r_t^{*}-\bar{r}_t^{*}\right)\right),}
#'where
...
#'TGARCH model (Zakoian, 1994), the more general
...
#'
#'@return
#'An object of S4-class \code{"fEGarch_fit_fitgarch"}
...
#'}
#'
fitgarch <- function(rt, orders = c(1, 1),
...
#'@export
#'@rdname show-methods
#'@aliases show,fEgarch_fit_fitgarch-method
setMethod(
  "show",
