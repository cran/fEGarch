figjrgarch_grab_pars <- function(theta, p, q, incl_mean, extra_par, skew_par) {
...
sigt_figjrgarch_R <- function(theta, x, p, q, p_ar, q_ma,
              incl_mean, extra_par, skew_par,
              trunc, presample, ...) {

  list2env(list(...), envir = environment())

  all_pars <- figjrgarch_grab_pars(
    theta = theta,
    p = p,
    q = q,
    incl_mean = incl_mean,
...
goal_fun_creator_figjrgarch <- function(
    theta,
    x,
    pdf_fun2,
    p,
...
#'}
#'
figjrgarch <- function(rt, orders = c(1, 1),
  ...
  inp_args[["model_type"]] <- "figjrgarch"
...
#'@export
#'@rdname show-methods
#'@aliases show,fEgarch_fit_figjrgarch-method
setMethod(
  "show",
