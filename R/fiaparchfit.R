fiaparch_grab_pars <- function(theta, p, q, incl_mean, extra_par, skew_par) {
  ...
  delta <- theta[[step2 + 2]]
  d <- theta[[step2 + 3]]
  shape <- if (extra_par) theta[[step2 + 4]] else 0
  skew <- if (skew_par) theta[[step2 + 4 + extra_par]] else 0
...
sigt_fiaparch_R <- function(theta, x, p, q, p_ar, q_ma,
              incl_mean, extra_par, skew_par,
              trunc, presample, ...) {

  list2env(list(...), envir = environment())

  all_pars <- fiaparch_grab_pars(
    theta = theta,
    p = p,
    q = q,
    incl_mean = incl_mean,
  ...
  x_adj <- c(rep(presample_val, presample), (abs(x_dm) - gamma * x_dm)^delta, rep(0, np2 - n))

  sigt <- (omega_sig + Re(utils::tail(utils::head(stats::fft(stats::fft(c_adj) * stats::fft(x_adj), inverse = TRUE), n), length(x_dm))) / np2)^(1 / delta)

  list(sigt = sigt, mu = mu, skew = skew, shape = shape, cmeans = rep(mu, length(sigt)))

}

goal_fun_creator_fiaparch <- function(
    theta,
    x,
    pdf_fun2,
    p,
...
#'
#'Fit a fractionally integrated APARCH model under the six
...
#'@param presample a numeric value indicating the pre-sample length for
#'initialization.
...
#'\deqn{\sigma_t^{\delta}=\omega+\left[1-\beta^{-1}(B)\phi(B)(1-B)^{d}\right]\left(\left|\varepsilon_t\right|-\gamma\varepsilon_t\right)^{\delta}.}
...
#'real-valued parameter and \eqn{\gamma \in (-1,1)} and \eqn{d \in [0,1]} as the
#'parameter for the level of integration. With \eqn{d = 0} the model reduces
#'to a short-memory APARCH, for \eqn{d=1} we have a full integration, and for
...
#'
#'The pre-sample values of \eqn{\left(\left|\varepsilon_t\right|-\gamma\varepsilon_t\right)^{\delta}}
#'are replaced by the in-sample arithmetic mean of
#'\deqn{\left(\left|r_t^{*} - \bar{r}_t^{*}\right|-
#'\gamma_0\left(r_t^{*}-\bar{r}_t^{*}\right)\right)^{\delta_0},}
#'where
#'\eqn{r_t^{*}} are considered as the observations and \eqn{\bar{r}_t^{*}} is the sample
#'mean of \eqn{r_t^{*}}. Initial values
#'for \eqn{\gamma_0} and
#'\eqn{\delta_0} as initial guesses for \eqn{\gamma} and \eqn{\delta} are obtained
...
#'\eqn{\phi}, \eqn{\beta}, \eqn{\gamma}, \eqn{\delta}, \eqn{d},
#'shape parameter,
...
#'
#'@return
#'An object of S4-class \code{"fEGarch_fit_fiaparch"}
...
#'}
#'
fiaparch <- function(rt, orders = c(1, 1),
...
#'@export
#'@rdname show-methods
#'@aliases show,fEgarch_fit_fiaparch-method
setMethod(
  "show",
  "fEGarch_fit_fiaparch",
  function(object) {
  x <- object
